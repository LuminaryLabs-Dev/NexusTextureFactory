        ...
        const MASK_VIEW_MODES = [
            { id: 'transparent', name: 'Transparent' },
            { id: 'bw', name: 'B/W' }
        ];
        const DEFAULT_MASK_VIEW_MODE = 'transparent';
        const DEFAULT_SET_GRID_COLUMNS = 5;
        const REFERENCE_MASK_RESOLUTION = 256.0;
        ...
        const FS_MORPH_PASS = `precision mediump float; varying vec2 vUv; uniform sampler2D u_sourceTexture; uniform float u_mode; uniform float u_radius; uniform float u_scale; uniform float u_offsetX; uniform float u_offsetY; float sampleSource(vec2 uv){ if (uv.x < 0.0 || uv.x > 1.0 || uv.y < 0.0 || uv.y > 1.0) return 0.0; return texture2D(u_sourceTexture, uv).a; } void main(){ vec2 uv = (vUv - 0.5) / max(0.001, u_scale) + 0.5 - vec2(u_offsetX, u_offsetY); int radius = int(clamp(floor(u_radius + 0.5), 0.0, 8.0)); vec2 t = vec2(1.0 / ${REFERENCE_MASK_RESOLUTION.toFixed(1)}); float outA = (u_mode < 0.5) ? 0.0 : 1.0; for (int y = -8; y <= 8; y++) { for (int x = -8; x <= 8; x++) { if (abs(x) > radius || abs(y) > radius) continue; float a = sampleSource(uv + vec2(float(x), float(y)) * t); if (u_mode < 0.5) outA = max(outA, a); else outA = min(outA, a); } } gl_FragColor = vec4(1.0, 1.0, 1.0, outA); }`;
        const FS_BLEND_PASS = `precision mediump float; varying vec2 vUv; uniform sampler2D u_prevTexture; uniform sampler2D u_currentTexture; uniform int u_blendMode; uniform float u_power; uniform float u_mult; float sampleTex(sampler2D tex, vec2 uv){ if (uv.x < 0.0 || uv.x > 1.0 || uv.y < 0.0 || uv.y > 1.0) return 0.0; return texture2D(tex, uv).a; } void main(){ float prevAlpha = sampleTex(u_prevTexture, vUv); float currentAlpha = clamp(pow(sampleTex(u_currentTexture, vUv), u_power) * u_mult, 0.0, 1.0); float finalAlpha = prevAlpha; if (u_blendMode == 0) finalAlpha = currentAlpha; else if (u_blendMode == 1) finalAlpha = max(0.0, prevAlpha - currentAlpha); else if (u_blendMode == 2) finalAlpha = prevAlpha * currentAlpha; else if (u_blendMode == 3) finalAlpha = min(1.0, prevAlpha + currentAlpha); else if (u_blendMode == 4) finalAlpha = max(prevAlpha, currentAlpha); else if (u_blendMode == 5) finalAlpha = min(prevAlpha, currentAlpha); gl_FragColor = vec4(1.0, 1.0, 1.0, finalAlpha); }`;
        ...
        const STEP_TYPES = {
            ...
            MORPH_DILATE_ERODE: { id: 104, name: "Morph Dilate/Erode", cat: "FILT", params: { p1: 0, p2: 2, p3: 0, p4: 0, p5: 0, p6: 0, p7: 0 }, controls: [{ key: 'p1', label: "Mode", type: 'select', options: [{ label: "Dilate", value: 0 }, { label: "Erode", value: 1 }] }, { key: 'p2', label: "Radius", type: 'slider', min: 0, max: 8, step: 1 }] },
            MORPH_OPEN_CLOSE: { id: 105, name: "Morph Open/Close", cat: "FILT", params: { p1: 0, p2: 2, p3: 0, p4: 0, p5: 0, p6: 0, p7: 0 }, controls: [{ key: 'p1', label: "Mode", type: 'select', options: [{ label: "Open (Smooth)", value: 0 }, { label: "Close (Fill)", value: 1 }] }, { key: 'p2', label: "Radius", type: 'slider', min: 0, max: 8, step: 1 }] },
            EDGE_SOBEL: { id: 106, name: "Edge Detect", cat: "FILT", customOpType: "shader", customCode: "float customOp(vec2 uv){ vec2 t = vec2(max(0.25, p4) / " + REFERENCE_MASK_RESOLUTION.toFixed(1) + "); float tl = samplePrev(uv + vec2(-t.x, -t.y)); float tc = samplePrev(uv + vec2(0.0, -t.y)); float tr = samplePrev(uv + vec2(t.x, -t.y)); float ml = samplePrev(uv + vec2(-t.x, 0.0)); float mr = samplePrev(uv + vec2(t.x, 0.0)); float bl = samplePrev(uv + vec2(-t.x, t.y)); float bc = samplePrev(uv + vec2(0.0, t.y)); float br = samplePrev(uv + vec2(t.x, t.y)); float gx = -tl - 2.0 * ml - bl + tr + 2.0 * mr + br; float gy = -tl - 2.0 * tc - tr + bl + 2.0 * bc + br; float g = length(vec2(gx, gy)) * p1; float a = smoothstep(p2, p2 + 0.05, g); if (p3 > 0.5) a = 1.0 - a; return clamp(a, 0.0, 1.0); }", params: { p1: 1.0, p2: 0.1, p3: 0, p4: 1.0, p5: 0, p6: 0, p7: 0 }, controls: [{ key: 'p1', label: "Strength", type: 'slider', min: 0, max: 5, step: 0.05 }, { key: 'p2', label: "Threshold", type: 'slider', min: 0, max: 1, step: 0.01 }, { key: 'p3', label: "Invert", type: 'slider', min: 0, max: 1, step: 1 }, { key: 'p4', label: "Width", type: 'slider', min: 0.25, max: 8, step: 0.25 }] },
            OUTLINE_ALPHA: { id: 107, name: "Outline", cat: "FILT", customOpType: "shader", customCode: "float customOp(vec2 uv){ vec2 t = vec2(1.0 / " + REFERENCE_MASK_RESOLUTION.toFixed(1) + "); float center = samplePrev(uv); float radius = clamp(p1, 0.0, 8.0); float maxN = 0.0; float minN = 1.0; for (int y = -8; y <= 8; y++) for (int x = -8; x <= 8; x++) { float dx = float(x); float dy = float(y); if (sqrt(dx*dx + dy*dy) > radius) continue; float a = samplePrev(uv + vec2(dx, dy) * t); maxN = max(maxN, a); minN = min(minN, a); } float outer = max(0.0, maxN - center); float inner = max(0.0, center - minN); float mode = floor(p3 + 0.5); float outA = outer; if (mode > 0.5 && mode < 1.5) outA = inner; if (mode >= 1.5) outA = max(outer, inner); return smoothstep(0.0, max(0.001, p2), outA); }", params: { p1: 2.0, p2: 0.1, p3: 2, p4: 0, p5: 0, p6: 0, p7: 0 }, controls: [{ key: 'p1', label: "Thickness", type: 'slider', min: 0, max: 8, step: 1 }, { key: 'p2', label: "Softness", type: 'slider', min: 0.01, max: 1, step: 0.01 }, { key: 'p3', label: "Mode", type: 'select', options: [{ label: "Outer", value: 0 }, { label: "Inner", value: 1 }, { label: "Both", value: 2 }] }] },
            ...
            DISTANCE_BANDS: { id: 109, name: "Distance Bands", cat: "FILT", customOpType: "shader", customCode: "float customOp(vec2 uv){ float a = samplePrev(uv); int radius = int(clamp(floor(max(1.0, p5) + 0.5), 1.0, 8.0)); vec2 t = vec2(1.0 / " + REFERENCE_MASK_RESOLUTION.toFixed(1) + "); float sum = 0.0; float count = 0.0; for (int y = -8; y <= 8; y++) for (int x = -8; x <= 8; x++) { if (abs(x) > radius || abs(y) > radius) continue; sum += samplePrev(uv + vec2(float(x), float(y)) * t); count += 1.0; } float avg = sum / max(1.0, count); float edge = abs(a - avg); float phase = fract(edge * max(1.0, p1) + p4); float band = smoothstep(p2, p2 + max(0.001, p3), phase) - smoothstep(p2 + p3, p2 + p3 + max(0.001, p3), phase); return clamp(band, 0.0, 1.0); }", params: { p1: 8, p2: 0.3, p3: 0.1, p4: 0.0, p5: 2.0, p6: 0, p7: 0 }, controls: [{ key: 'p1', label: "Frequency", type: 'slider', min: 1, max: 64, step: 1 }, { key: 'p2', label: "Band Start", type: 'slider', min: 0, max: 1, step: 0.01 }, { key: 'p3', label: "Band Width", type: 'slider', min: 0.01, max: 0.5, step: 0.01 }, { key: 'p4', label: "Phase", type: 'slider', min: 0, max: 1, step: 0.01 }, { key: 'p5', label: "Sample Radius", type: 'slider', min: 1, max: 8, step: 1 }] },
        ...
        const OPERATION_EXPLANATIONS = {
            ...
            MORPH_OPEN_CLOSE: "Applies true open/close morphology passes in normalized mask space.",
...
return Math.max(0, Math.min(1, 1 - d * 2.2));`;
        ...
        const hammingDistance = (a, b) => {
	        ...
	        class TextureEngine {
	            ...
	            ensureScratchTextures(requiredCount) {
	                if (this.scratchTextures.length >= requiredCount) return;
	                for (let i = this.scratchTextures.length; i < requiredCount; i++) {
	                    this.scratchTextures.push(this.createFBO());
	                }
	            }
	            getBaseProgramMeta() {
...
void main() {
    ...
    gl_FragColor = vec4(1.0, 1.0, 1.0, finalAlpha);
	            ...
	            getMorphPassMeta() {
	                if (this.morphPassMeta) return this.morphPassMeta;
	                const program = this.createProgram(VS_SOURCE, FS_MORPH_PASS);
	                if (!program) return null;
	                const gl = this.gl;
	                this.morphPassMeta = {
	                    program,
	                ...
	                return this.morphPassMeta;
	            }
	            getBlendPassMeta() {
	                if (this.blendPassMeta) return this.blendPassMeta;
	                const program = this.createProgram(VS_SOURCE, FS_BLEND_PASS);
	                if (!program) return null;
	                const gl = this.gl;
	                this.blendPassMeta = {
	                    program,
	                    ...
	                    locs: {
	                        u_prevTexture: gl.getUniformLocation(program, "u_prevTexture"),
	                        u_currentTexture: gl.getUniformLocation(program, "u_currentTexture"),
	                        u_blendMode: gl.getUniformLocation(program, "u_blendMode"),
	                        u_power: gl.getUniformLocation(program, "u_power"),
	                        u_mult: gl.getUniformLocation(program, "u_mult")
	                    }
	                };
	                return this.blendPassMeta;
	            }
	            renderMorphPass(sourceTexture, dest, mode, radius, universal) {
	                const meta = this.getMorphPassMeta();
	                if (!meta) return;
	                const gl = this.gl;
	                this.bindQuadProgram(meta);
	                gl.bindFramebuffer(gl.FRAMEBUFFER, dest.fbo);
	                gl.viewport(0, 0, this.width, this.height);
	                gl.clear(gl.COLOR_BUFFER_BIT);
	                gl.activeTexture(gl.TEXTURE0);
	                gl.bindTexture(gl.TEXTURE_2D, sourceTexture);
	                gl.uniform1i(meta.locs.u_sourceTexture, 0);
	                gl.uniform1f(meta.locs.u_mode, mode);
	                gl.uniform1f(meta.locs.u_radius, radius);
	                gl.uniform1f(meta.locs.u_scale, universal?.scale ?? 1.0);
	                gl.uniform1f(meta.locs.u_offsetX, universal?.offsetX ?? 0.0);
	                gl.uniform1f(meta.locs.u_offsetY, universal?.offsetY ?? 0.0);
	                gl.drawArrays(this.gl.TRIANGLES, 0, 6);
	            }
	            renderBlendPass(prevTexture, currentTexture, dest, step) {
	                const meta = this.getBlendPassMeta();
	                if (!meta) return;
	                const gl = this.gl;
	                this.bindQuadProgram(meta);
	                gl.bindFramebuffer(gl.FRAMEBUFFER, dest.fbo);
	                ...
	                gl.uniform1i(meta.locs.u_prevTexture, 0);
	                gl.activeTexture(gl.TEXTURE1);
	                gl.bindTexture(gl.TEXTURE_2D, currentTexture);
	                gl.uniform1i(meta.locs.u_currentTexture, 1);
	                gl.uniform1i(meta.locs.u_blendMode, step?.blendMode ?? 0);
	                gl.uniform1f(meta.locs.u_power, step?.universal?.power ?? 1.0);
	                gl.uniform1f(meta.locs.u_mult, step?.universal?.mult ?? 1.0);
	                gl.drawArrays(this.gl.TRIANGLES, 0, 6);
	            }
	            renderMorphologyStep(step, prevTex, dest) {
	                this.ensureScratchTextures(2);
	                const scratchA = this.scratchTextures[0];
	                const scratchB = this.scratchTextures[1];
	                const radius = step?.params?.p2 ?? 0;
	                const mode = Math.floor((step?.params?.p1 ?? 0) + 0.5);
	                const isOpenClose = step?.typeDef?.id === 105;
	                if (isOpenClose) {
	                    const firstMode = mode < 0.5 ? 1.0 : 0.0;
	                    const secondMode = mode < 0.5 ? 0.0 : 1.0;
	                    this.renderMorphPass(prevTex, scratchA, firstMode, radius, step.universal);
	                    this.renderMorphPass(scratchA.texture, scratchB, secondMode, radius, { scale: 1.0, offsetX: 0.0, offsetY: 0.0 });
	                    this.renderBlendPass(prevTex, scratchB.texture, dest, step);
	                    return;
	                }
	                const passMode = mode < 0.5 ? 0.0 : 1.0;
	                this.renderMorphPass(prevTex, scratchA, passMode, radius, step.universal);
	                this.renderBlendPass(prevTex, scratchA.texture, dest, step);
	            }
	            ...
	            renderStack(steps, options = {}) {
	                ...
	                for (let index = 0; index < steps.length; index++) {
	                    ...
	                    if (step.active && (step.typeDef.id === 104 || step.typeDef.id === 105)) {
	                        this.renderMorphologyStep(step, prevTex, dest);
	            ...
	            pixelsToCanvas(pixels, options = {}) {
	                const mode = options.mode === 'bw' ? 'bw' : DEFAULT_MASK_VIEW_MODE;
	                ...
	                for (let y = 0; y < this.height; y++) {
	                    for (let x = 0; x < this.width; x++) {
	                        ...
	                        if (mode === 'bw') {
	                            imageData.data[destIdx] = alpha;
	                            imageData.data[destIdx + 1] = alpha;
	                            imageData.data[destIdx + 2] = alpha;
	            ...
	            getTextureCanvas(textureIndex, options = {}) {
	                const pixels = this.readPixels(textureIndex);
	                return this.pixelsToCanvas(pixels, options);
	            }
	            async getTextureCanvasAndBlob(textureIndex, mimeType = 'image/png', quality, options = {}) {
	                const canvas = this.getTextureCanvas(textureIndex, options);
	                const blob = await new Promise((resolve) => {
	                    canvas.toBlob((nextBlob) => resolve(nextBlob || new Blob()), mimeType, quality);
	                });
	                return { canvas, blob };
	            }
	            getTextureUrl(textureIndex, options = {}) {
	                const canvas = this.getTextureCanvas(textureIndex, options);
	                return canvas.toDataURL();
	            }
	            async getTextureBlob(textureIndex, mimeType = 'image/png', quality, options = {}) {
	                const canvas = this.getTextureCanvas(textureIndex, options);
